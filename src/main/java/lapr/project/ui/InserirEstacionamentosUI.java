...
public class InserirEstacionamentosUI {
    ...
    public void inserirEstacionamento() {
        ...
        if (farmacia != null) {
            ...
            int numMax = controller.getParqueByID(idParque).getNumeroMaximo();
            if (lstEstacs.size() != numMax) {
                for (int i = 0; i < numMax; i++) {
                    ...
                    int numLote = i + 1;
                    Estacionamento estac = controller.novoEstacionamento(numLote, carregador, idParque);
                    lestac.add(estac);
            ...
            System.out.println("Registo de Estacionamentos cancelado!");
            System.out.println("NIF da farmácia introduzido é inválido!");
        }
    }
}
